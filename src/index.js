...
const mapObj = (obj, fn) => {
  const mappedAsArr = Object.entries(obj).map(
    ([k, v]) => fn(k, v)
  );
...
const flattenedConfigKeysHash = flattenObjHash(
  defaultConfig
);
const isPlainConfigObj = (val) => {
  return (
    ...
    Object.keys(val).every((k) =>
      flattenedConfigKeysHash.hasOwnProperty(k)
    )
  );
};
const mergeConfigs = (
  ...
  const final = mapObj(
    ...
    (k, v) => {
      ...
      return [
        k,
        inputConfigVal.hasOwnProperty(k)
          ? inputConfigVal[k]
          : v,
      ];
    }
...
const getPrefixSegments = (levelName, config) => {
  ...
  const prefixElements = [
    ...
    config.prefix.includeTime &&
      config.prefix.getCurrentTimeString(),
    ...config.prefix.getRestOfPrefix(),
  ];

  const formatChar = config.prefix.colors
    ? "%c"
    : "";
  const prefixStr =
    formatChar +
    config.prefix.format(prefixElements);
  const css = config.prefix.colors
    ? `color: ${config.prefix.colors[levelName]};`
    : "";
  const prefixSegments = css
    ? [prefixStr, css]
    : [prefixStr];
  ...
  const makeLevelLogger = (levelName, intVal) => {
    return (...baseLogArgs) => {
      ...
      const hasAdditionalConfig =
        isPlainConfigObj(baseLogArgs[0]);
      ...
      if (
        !config.enabled ||
        intVal > normalizeLevel(config.level)
      ) {
        return;
      }
      const metadata = invokeIfFunction(
        config.metadata
      );
      ...
      const asString = config.formatLogSegments(
        allLogSegments
      );
      if (
        !config.filter({
